...
export const Card = styled.li<{ index: number; isDarkMode: boolean }>`
  ...
  &:last-child {
    border-bottom: 1px solid ${({ isDarkMode }) => getModeBorder(!isDarkMode)};
    z-index: 1000;
  }
...
export const ImageWrapper = styled.div<{ isDarkMode: boolean }>`
  flex: 1;
  display: flex;
  border-right: 1px solid ${({ isDarkMode }) => getModeBorder(!isDarkMode)};
...
export const CardTitle = styled.h3<{ isDarkMode: boolean }>`
  ...
  padding-block: min(4.2vw, 47px) min(3.9vw, 45px);
  border-bottom: 1px solid ${({ isDarkMode }) => getModeBorder(!isDarkMode)};
